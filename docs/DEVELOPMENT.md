...
仍需继续：admin 页面接入统一 query UX、统一 loading 细节、前端自动化测试。
...
- 新增 Next.js `app/error.tsx` 全局错误边界。
- 前端页面运行时异常会显示友好的错误页，不再直接白屏。
...
- `/search` 搜索、问答的进行中、成功和失败提示改用全局 toast。
- `/preferences` 偏好保存成功/失败提示改用全局 toast。
...
- `/documents` 文档列表改为 `useQuery` 加载和缓存。
- `/documents` 文档详情改为按 `selectedId` 使用 `useQuery` 加载。
...
- `/preferences` 偏好加载改为 `useQuery`。
- `/preferences` 保存偏好改为 `useMutation`。
- 保存成功后更新 preferences query cache，并 invalidate recommendations/dashboard。
- 偏好页初次加载显示 skeleton。
...
### 第三十八阶段：统一 Query UX 第一批

本阶段已完成：

- 新增 `apps/web/components/QueryState.tsx`，提供 `useQueryErrorToast` 和 `SkeletonList`。
- dashboard、documents、recommendations、feedback、preferences 已接入统一 query 错误 toast。
- 已迁移页面的初始 skeleton 列表改为复用 `SkeletonList`，减少重复页面代码。
- dashboard 和 recommendations 不再额外渲染页面内红色错误块，query 错误统一由 toast 提示。
...
- 前端工程质量还需继续补强：API client 已补 timeout、AbortController 和统一 401，React Query 已接入并迁移 dashboard/recommendations/documents/preferences/search，统一 query error toast 第一批、全局 toast 第二批、全局错误页、页面级 skeleton 第一批和 documents 页面体验收口已完成；仍缺 admin 页面接入统一 query UX、loading 细节统一和前端自动化测试。
