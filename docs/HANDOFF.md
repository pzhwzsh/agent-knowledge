...
3. 前端工程化：admin 页面接入统一 query UX、统一 loading 细节、更多页面级 skeleton、自动化测试和依赖版本锁定。
4. worker 将 job 标记 running。
5. 文本直接使用；URL 先校验安全性再抓取。
6. 前端工程化增强：admin 页面接入统一 query UX、统一 loading 细节、页面级 skeleton、自动化测试和依赖版本锁定。
...
- 前端 API client 已补 timeout、AbortController 和统一 401；React Query 已接入并迁移 dashboard/recommendations/documents/preferences/search，全局 toast 第二批、全局错误页、页面级 skeleton 第一批、documents 页面体验收口和统一 query UX 第一批已完成；仍缺 admin 页面接入、loading 细节统一和前端自动化测试。
...
- 已完成全局 toast 第二批：search、preferences、admin feedback 和 admin audit 关键提示已接入全局 toast。
- 已完成全局错误页第一批：新增 `app/error.tsx`，页面异常时提供重试和返回仪表盘入口。
- 已完成页面级 skeleton 第一批：dashboard、recommendations 和 feedback 初次加载显示骨架屏。
- 已完成 documents 页面体验收口第一批：列表加载 skeleton，加载/删除提示接入全局 toast。
- 已完成 documents React Query 迁移第一批：文档列表、详情和删除操作由 query/mutation 管理。
- 已完成 preferences React Query 迁移第一批：偏好加载/保存由 query/mutation 管理，保存后刷新推荐和仪表盘缓存。
