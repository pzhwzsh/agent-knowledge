...
export default function SearchPage() {
  ...
  async function search(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!auth.token || !query.trim()) return; notify("搜索中...", "info"); try { const data = await apiRequest<{ results: SearchResult[] }>("/api/search", { method: "POST", body: JSON.stringify({ query, limit: 8 }) }, auth.token); setResults(data.results); notify(`找到 ${data.results.length} 条相关片段。`, "success"); } catch (error) { notify(error instanceof Error ? error.message : "搜索失败", "error"); } }
  async function ask(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!auth.token || !question.trim()) return; notify("正在问答...", "info"); try { setChat(await apiRequest<ChatResponse>("/api/chat", { method: "POST", body: JSON.stringify({ question, limit: 5 }) }, auth.token)); notify("问答完成。", "success"); } catch (error) { notify(error instanceof Error ? error.message : "问答失败", "error"); } }
