...
二期第一批已完成：安全收口、任务重放、推送控制、前端 API client 工程化、生产 compose、模型辅助推荐、使用反馈闭环和反馈处理后台第一版、审计日志查询第一版和服务端登出撤销第一版和撤销 token 清理任务第一版。当前项目仍不是完整商业化生产版本，三期会继续补 CI/CD、管理后台、审计日志、React Query/toast/error boundary、浏览器插件、视频字幕、Rerank 和多模型路由等能力。
...
- PDF 完整解析。
- 浏览器插件。
- 完整管理后台仍未完成；当前仅有反馈处理后台第一版、审计日志查询第一版和服务端登出撤销第一版和撤销 token 清理任务第一版。
- 知识库导出。
