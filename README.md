...
如果当前电脑没有 Docker，可以先用 SQLite 预览登录、注册和基础前端页面。该模式只用于本地体验，不替代 PostgreSQL/pgvector 的正式验证。

后端：

```powershell
cd apps/api
Copy-Item .env.local-preview.example .env
...
然后打开 `http://localhost:3000`。首次启动后端时，`LOCAL_PREVIEW_AUTO_CREATE=true` 会在 SQLite 文件 `apps/api/local_preview.db` 中自动建表。
