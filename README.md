...
- 前端已接入后端：支持注册、登录、token 保存、退出登录、登录过期统一处理、API 超时控制、React Query 基础数据缓存、仪表盘、文档、推荐箱、搜索问答、偏好设置、快速采集和反馈维修台；管理员可进入反馈处理后台查看全部反馈并更新处理状态，也可以进入审计日志页面查看敏感操作记录。

## 二期第一批状态

二期第一批已完成：安全收口、任务重放、推送控制、前端 API client 工程化、生产 compose、模型辅助推荐、使用反馈闭环和反馈处理后台第一版和审计日志查询第一版。当前项目仍不是完整商业化生产版本，三期会继续补 CI/CD、管理后台、审计日志、React Query/toast/error boundary、浏览器插件、视频字幕、Rerank 和多模型路由等能力。
...
- PDF 完整解析。
- 浏览器插件。
- 完整管理后台仍未完成；当前仅有反馈处理后台第一版和审计日志查询第一版。
- 知识库导出。
