...
- 前端已接入后端：支持注册、登录、token 保存、服务端登出撤销、登录过期统一处理、API 超时控制、React Query 基础数据缓存、全局 toast 提示、全局错误页、页面级 skeleton、仪表盘、文档、推荐箱、搜索问答、偏好设置、快速采集和反馈维修台；管理员可进入反馈处理后台查看全部反馈并更新处理状态，也可以进入审计日志页面查看敏感操作记录。
